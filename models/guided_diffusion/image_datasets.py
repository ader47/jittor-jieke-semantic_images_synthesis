...
def load_data(
    ...
    if is_feature:
        dataset_path = []
        if not local:
            dataset_path.append(os.path.join(data_dir, 'images.npy'))
            dataset_path.append(os.path.join(data_dir, 'labels.npy'))

        if is_train:
            dataset = FeatureBase(dataset_path, index='arr_0', shard=jittor.rank,
                                  num_shards=jittor.world_size, is_train=is_train, is_kl=is_kl)
        else:
            dataset_path[1] = os.path.join(data_dir, 'val_B_labels_resized')

            dataset = FeatureBase(dataset_path, index='arr_0', shard=jittor.rank,
                                  num_shards=jittor.world_size, is_train=is_train, image_size=(128, 96), is_kl=is_kl)
    else:
        if not data_dir:
        ...
        if dataset_mode == 'cityscapes':
            all_files = _list_image_files_recursively(
                os.path.join(data_dir, 'leftImg8bit', 'train' if is_train else 'val'))
            labels_file = _list_image_files_recursively(
                os.path.join(data_dir, 'gtFine', 'train' if is_train else 'val'))
            ...
            all_files = _list_image_files_recursively(
                os.path.join(data_dir, 'images', 'training' if is_train else 'validation'))  # image file names
            classes = _list_image_files_recursively(os.path.join(data_dir, 'annotations',
                                                                 'training' if is_train else 'validation'))  # label file name  # todo the label data threshold
            instances = None
        ...
        elif dataset_mode == 'jittor':
            all_files = _list_image_files_recursively(
                os.path.join(data_dir, 'imgs' if is_train else 'val_A_labels_resized'))  # image file names
            classes = _list_image_files_recursively(
                os.path.join(data_dir, 'labels' if is_train else 'val_A_labels_resized'))  # label file name
            instances = None
        else:
...
class ImageDataset(Dataset):
    def __init__(
        ...
        self.local_images = image_paths[shard:][::num_shards]  # 有间隔, 防止取到相同的数据
    ...
    def __getitem__(self, idx):
        ...
        # todo training remove this line
        # out_dict['class_path'] = class_path
...
class FeatureBase(Dataset):
    def __init__(self, dataset_path, index, is_train, shard=0, num_shards=1, image_size=(128, 96), is_kl=True):
        super().__init__()
        ...
        if is_train:
            if False:
                self.dataset_1 = np.load(dataset_path[0], mmap_mode="r")[shard:][::num_shards]
                self.dataset_2 = np.load(dataset_path[1], mmap_mode="r")[shard:][::num_shards]
            # https://cg.cs.tsinghua.edu.cn/jittor/tutorial/2020-5-2-16-44-distributed/
            # 数据自动并行分发，应该不需要手动写分发
            else:
                self.dataset_1 = np.load(dataset_path[0], mmap_mode="r")
                self.dataset_2 = np.load(dataset_path[1], mmap_mode="r")
        else:
            print("Evaluation ...")
            self.root_path = dataset_path[1]
            self.labels = os.listdir(self.root_path)

    def __len__(self):
        if self.is_train:
            return self.dataset_2.shape[0]
        else:
    ...
    def __getitem__(self, item):
        ...
        if self.is_train:
            out_dict = {}
            label = self.dataset_2[item]
            out_dict['label'] = jittor.float32(np.array(label[None,]))
            out_dict['label_ori'] = jittor.float32(np.array(self.dataset_2[item]))
            if self.is_kl:
                mean, logvar = jittor.chunk(jittor.float32(self.dataset_1[item]), 2, dim=0)
                std = jittor.exp(0.5 * logvar)
                out = (mean + std * jittor.randn(mean.shape))
            else:
                out = jittor.float32(np.array(self.dataset_1[item]))
            if not self.flip:
                return self.dataset[item], out_dict
            else:
                return out, out_dict
        else:
            out_dict = {}
            out_dict['root_path'] = self.root_path
            out_dict['file_name'] = self.labels[item]
            label = Image.open(os.path.join(out_dict['root_path'], out_dict['file_name']))
            label = label.resize(self.image_size, Image.NEAREST)
            label = np.array(label)
            out_dict['label'] = label[None,]
            out_dict['label_ori'] = label
            return label, out_dict
...
def random_crop_arr(pil_list, image_size, min_crop_frac=0.8, max_crop_frac=1.0):
    ...
    return arr_image, \
        arr_class, \
        arr_instance if arr_instance is not None else None
